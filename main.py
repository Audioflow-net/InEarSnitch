...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        btn_l.setStyleSheet("QPushButton { background-color: #222; color: #888; border: 1px solid #444; border-top-left-radius: 6px; border-bottom-left-radius: 6px; border-right: none; padding: 12px 28px; font-weight: bold; font-size: 14px; } QPushButton:checked { background-color: #16a34a; color: white; border-color: #16a34a; }")
        btn_r.setStyleSheet("QPushButton { background-color: #222; color: #888; border: 1px solid #444; border-top-right-radius: 6px; border-bottom-right-radius: 6px; padding: 12px 28px; font-weight: bold; font-size: 14px; } QPushButton:checked { background-color: #dc2626; color: white; border-color: #dc2626; }")
    ...
    def _run_level_calibration(self):
        ...
        for amp in steps:
            try:
                ...
                try:
                    rec = sd.playrec(tone_stereo, samplerate=self.audio_engine.sample_rate,
                                     channels=1, device=(self.selected_in_idx, self.selected_out_idx),
                                     blocking=True)
                except Exception:
                    in_chans = sd.query_devices(self.selected_in_idx)['max_input_channels']
                    rec = sd.playrec(tone_stereo, samplerate=self.audio_engine.sample_rate,
                                     channels=in_chans, device=(self.selected_in_idx, self.selected_out_idx),
                                     blocking=True)
                    rec = rec[:, 0:1]
                sd.wait()
