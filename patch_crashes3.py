...
# Fix 1: json.loads to np.frombuffer
code = code.replace(
    "np.array(json.loads(row[0]))",
    "np.frombuffer(row[0], dtype=np.float64)"
)
code = code.replace(
    "np.array(json.loads(row[1]))",
    "np.frombuffer(row[1], dtype=np.float64)"
)
code = code.replace(
    "np.array(json.loads(row[2]))",
    "np.frombuffer(row[2], dtype=np.float64)"
)

# Fix 2: re-inject update_analysis_view
insert_point = r'(    def on_measurement_finished\(self, freqs, mag, phase, ir, channel\):)'
