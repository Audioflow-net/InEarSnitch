...
new_method = """    def style_tab(self, btn, active):
        bg_inactive = theme.get_color('bg_main') if theme.CURRENT_MODE == 'dark' else theme.get_color('bg_hover')
        if active:
            ...
            btn.setStyleSheet(f"color: {theme.get_color('text_secondary')}; font-weight: bold; background-color: {bg_inactive}; border-radius: 4px; padding: 10px 20px; border: 1px solid transparent; border-bottom: 2px solid transparent;")
"""
