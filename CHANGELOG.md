...
## V32 - 2026-09-25
* **Bauteil:** TPU Insert & PETG Chassis (Logo-Ecke)
* **Maße (Alt vs. Neu):** Logo-Position von `X=18.15, Y=80.75` auf extrem symmetrische Ecke `X=12.0, Y=84.0` verschoben. Tiefe des Logos im TPU von 23.12mm (kompletter Turm-Durchbruch) auf 2.0mm reduziert (Einlege-Medaillon).
* **Formen-Änderung:** Der riesige, durchgehende ovale Logo-Turm (als Unterbau im PETG und Loch im TPU) wurde komplett gelöscht. Das TPU ist in der Ecke jetzt bis kurz unter die Oberfläche völlig massiv (Infill). Oben auf der TPU-Fläche befindet sich nur noch eine 2mm flache, exakte Negativ-Ausstanzung des Logos. Das PETG-Chassis bekommt anstelle des Turms den runden Standard-Zentrierstift (`[11.0, 77.0]`) zurück. Das Logo wird nun als separates, 2mm flaches Plättchen (`logo_medallion()`) gedruckt.
* **Grund:** Physikalisches Limit von FDM. Der tiefe, 12mm breite Hohlraum im TPU für den alten Turm führte dazu, dass der Drucker die "Inseln" des Logos (z.B. den Augenschlitz) mitten in der Luft drucken musste. Dies verursachte schwere Hänge-Probleme (Bridging-Fail) und TPU-Fäden. Die neue Medaillon-Lösung bietet einen massiven TPU-Boden, eliminiert alle Bridging-Probleme und ermöglicht ein perfekt bündiges 2-Farben-Inlay ohne Hohlräume.
